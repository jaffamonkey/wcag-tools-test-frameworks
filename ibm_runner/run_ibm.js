...
function extractFindings(report) {
  let list = [];
  if (Array.isArray(report?.results)) list = report.results;
  else if (Array.isArray(report?.issues)) list = report.issues;
  
  // Explicitly isolate hard violations only
  return list.filter(item => {
    const value = String(item.value || item.level || '').toLowerCase();
...
${bodyHtml}
    ...
    // If report didn't process correctly, fail safe with an error block instead of crashing writeFileSync
    if (!report) {
      throw new Error('IBM Checker returned an empty or invalid report object.');
    }

    const findings = extractFindings(report);
    
    // Explicitly clamp report payload properties to your violations array
    if (report.results) {
    ...
    fs.writeFileSync(jsonPath, JSON.stringify(report, null, 2), 'utf8');
    console.log(`Saved IBM report: ${jsonPath} (${findings.length} violations found)`);
...
(async () => {
  // FIXED: Restored array index selection to pick the path string passed via CLI
  const jobDir = process.argv[2]; 
  if (!jobDir) {
    throw new Error('Usage: node run_ibm.js <job_dir>');
  }
  ...
  // Programmatic global override configuration for aChecker instantiation
  if (aChecker.AConfig) {
    aChecker.AConfig({
      policies: ['WCAG_2_2'],
      ruleArchive: 'latest',
