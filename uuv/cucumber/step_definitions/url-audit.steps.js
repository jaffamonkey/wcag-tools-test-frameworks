...
async function runRichFullPageCheck(url) {
  ...
  this.scenarioName = `Full page check for ${getPageReference(url)}`;
  this.pageErrors = [];
  this.consoleMessages = [];
  this.failedRequests = [];
  this.report = null;
  ...
  const summary = {
    ...
    finalUrl,
    title,
    bodyVisible: domStats.bodyVisible,
  ...
  if (findings.some((f) => f.severity === 'high')) {
    throw new Error(findings.map((f) => `[${f.severity}] ${f.message}`).join(' | '));
  }
}
...
function buildReportFileName(input) {
  ...
  try {
    ...
    const lastPart = parts[parts.length - 1] || url.hostname || 'report';
