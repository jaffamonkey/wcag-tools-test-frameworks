...
function buildContextOptions(storageStatePath) {
  ...
  if (storageStatePath && fs.existsSync(storageStatePath)) {
    ...
    console.log(`axe-core using storage state: ${storageStatePath}`);
  } else {
    console.log("axe-core running without storage state for public job");
  }
  return options;
}
...
async function main() {
  ...
  const urls = fs
    .readFileSync(path.join(jobDir, "input", "urls.txt"), "utf-8")
    .split(/\r?\n/)
    .map(x => x.trim())
    .filter(Boolean);
  ...
  try {
    for (const url of urls) {
      ...
      try {
        ...
        fs.writeFileSync(
          path.join(reportsDir, `${safeSlug(url)}.json`),
